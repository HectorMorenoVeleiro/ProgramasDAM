...
public class Ejercicio15 {
    ...
    public static void main(String[] args) {
        ...
        String notaInicialFinal = null;
        int melodyLength = (int) (Math.random() * 6) * 4 + 4;
        for (int i = 1; i <= melodyLength-1; i++) {
            int notaASonar = (int) (Math.random() * notas.length);
            System.out.print(notas[notaASonar] + " ");
            if (i % 4 == 0)
                System.out.println(" | ");
            if (i==1)
                notaInicialFinal = notas[notaASonar];
        }
        System.out.println(notaInicialFinal + " |");
    }
}
